...
# Start full dev stack (server + queue + log watcher + Vite)
composer run dev
...
# Format PHP files after editing
...
Pages under `resources/views/pages/` with a `⚡` filename prefix use Livewire Volt anonymous classes — the PHP class is declared inline at the top of the blade file using `new #[...] class extends Component { ... }; ?>` before the HTML template. Routes register these with `Route::livewire('path', 'pages::section.name')` (using the `pages::` view namespace). See `resources/views/pages/settings/⚡profile.blade.php` for a canonical example.

### Authentication

Auth is handled entirely by **Fortify** (not Laravel's built-in auth scaffolding). `FortifyServiceProvider` registers all auth views and actions. The `User` model implements `PasskeyUser` and uses `PasskeyAuthenticatable` + `TwoFactorAuthenticatable` — passkeys and 2FA are enabled out of the box.

### Validation Concerns

Shared validation rule sets live in `app/Concerns/` (e.g., `ProfileValidationRules`, `PasswordValidationRules`) as traits. Livewire components use these traits and call the rule methods in `$this->validate(...)`.

### Database

- Local development: SQLite (`database/database.sqlite`)
- Tests: in-memory SQLite (configured in `phpunit.xml`); all Feature tests use `RefreshDatabase`

### Frontend

Tailwind CSS v4 (PostCSS-free, Vite plugin), Flux UI components (`<flux:*>`), Alpine.js for client-side interactivity. Custom Flux component overrides live in `resources/views/flux/`. Passkey JS is a separate entry point (`resources/js/passkeys.js`).
