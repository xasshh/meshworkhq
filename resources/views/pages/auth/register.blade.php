<x-layouts::auth :title="__('Create account')">
    <div class="flex flex-col gap-6 animate-fade-in-up">
        <div class="text-center">
            <h1 class="font-display text-2xl text-ink">Create your account</h1>
            <p class="text-sm text-ink-soft mt-2">Free to start, with no commission on completed work.</p>
        </div>
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-4"
            ...
            <div>
                <p class="text-[11px] font-bold text-ink-faint uppercase tracking-widest mb-2">I am joining as</p>
                <div class="relative grid grid-cols-2 bg-slate-100/90  p-1">
                    {{-- Sliding pill --}}
                    <div class="absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-white  shadow-sm ring-1 ring-slate-200/70 transition-transform duration-300 ease-out"
                         :class="role === 'professional' ? 'translate-x-full' : 'translate-x-0'"></div>

                    <button type="button" @click="role = 'client'"
                        class="relative z-10 py-2.5 px-3 text-center transition-colors duration-200">
                        <span class="block text-xs font-bold leading-tight"
                              :class="role === 'client' ? 'text-ink' : 'text-ink-faint'">Hiring Client</span>
                        <span class="block text-[10px] mt-0.5 font-medium"
                              :class="role === 'client' ? 'text-ink-soft' : 'text-ink-faint'">Post briefs, find talent</span>
                    </button>

                    <button type="button" @click="role = 'professional'"
                        class="relative z-10 py-2.5 px-3 text-center transition-colors duration-200">
                        <span class="block text-xs font-bold leading-tight"
                              :class="role === 'professional' ? 'text-brand-deep' : 'text-ink-faint'">Professional</span>
                        <span class="block text-[10px] mt-0.5 font-medium"
                              :class="role === 'professional' ? 'text-ink-soft' : 'text-ink-faint'">Receive leads, pitch clients</span>
                    </button>
                </div>
                @error('role')
            ...
            <div x-show="role === 'professional'" x-cloak
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" @click="addSkill(skillInput)"
                        class="px-3 py-2 bg-ink text-white text-xs font-semibold  hover:bg-ink-deep transition-colors duration-150">
                        Add
                    </button>
                </div>
                ...
                <div class="flex flex-wrap gap-1.5 mt-2 min-h-6">
                    <template x-for="(skill, i) in skills" :key="i">
                        <span class="inline-flex items-center gap-1 bg-ink text-white text-[11px] font-medium px-2.5 py-1 rounded-full animate-fade-in-up">
        ...
        <p class="text-sm text-center text-ink-faint">
            ...
            <a href="{{ route('login') }}" wire:navigate class="font-semibold text-ink hover:text-brand-deep transition-colors duration-150">Sign in</a>
        </p>
    </div>
</x-layouts::auth>
